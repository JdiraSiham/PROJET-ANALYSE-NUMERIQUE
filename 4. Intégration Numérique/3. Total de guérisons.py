...
I.append(I_t) 
R.append(R_t) 
# Calcul du taux de guérison dR/dt = a * I 
dR_dt = [a * I[i] for i in range(len(jours))] 
# Méthode des trapèzes 
Rtotal_trap = 0 
for i in range(len(dR_dt) - 1): 
Rtotal_trap += (dR_dt[i] + dR_dt[i + 1]) / 2 * dt 
# Méthode de Simpson 
Rtotal_simpson = simpson(dR_dt, dx=dt) 
print("Nombre total estimé de guérisons (méthode des trapèzes) :", 
int(Rtotal_trap)) 
print("Nombre total estimé de guérisons (méthode de Simpson)  :", 
int(Rtotal_simpson)) 
# Affichage graphique 
plt.figure(figsize=(10, 5)) 
plt.plot(jours, dR_dt, label="dR/dt (guérisons)", color='green') 
plt.xlabel("Jours") 
plt.ylabel("dR/dt") 
plt.title("Taux de guérison au cours du temps") 
plt.grid(True) 
plt.legend() 
plt.show()
